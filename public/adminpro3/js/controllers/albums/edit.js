app.controller('EditAlbumCtrl', function ($scope,$state,$stateParams,Upload,Cloud,Api) {
  ...
  var imgs = [];
	...
	function loadData(){
	    $scope.spinner.on();
	    Api.getAlbum($stateParams.id).success(function (album) {
	      $scope.Albums = album;
  ...
  $scope.create = function(album, file, cover, audio,audio2){
      ...
      Api.putAlbum(album._id, album).success( function(data){
        var state = true;
        var state2 = true;
        var stateCover = true;
        var stateImg = true;
          ...
          stateCover = false;
           Cloud.upload({
            ...
            .then(function (resp) { //upload function returns a promise
                    Api.putAlbum(album._id,{cover: resp.data.message}).success(function (){ 
                        stateCover = true;
        ...
        if( audio != null || audio != undefined){
          ...
          state = false;
           Cloud.upload({
            ...
            .then(function (resp) { //upload function returns a promise
                    Api.putAlbum(album._id,{audio: resp.data.message}).success(function (){
                        state = true;
                        if(state2 &&  state && stateImg && stateCover){
                          console.log('Entro audio');
                          $scope.spinner.off();
          ...
          angular.forEach(file, function(value, key) {
            ...
            .then(function (resp) { //upload function returns a promise
                  ...
                  if(file.length == imgs.length){
                    Api.putAlbum(album._id,{images: imgs}).success(function (){ 
                            stateImg = true;
                            if(state2 &&  state && stateImg && stateCover){
