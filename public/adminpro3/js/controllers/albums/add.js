app.controller('AddAlbumCtrl', function ($scope,$state,Upload,Cloud,Api) {
 ...
 $scope.create = function(album, file, cover, audio, audio2){
    ...
    var stateImg = true;
      ...
      Api.postAlbums(album).success( function(data){
        var state = true;
        var state2 = true;
        var stateCover = true;
        var stateImg = true;
          ...
          stateCover = false;
           Cloud.upload({
            ...
            .then(function (resp) { //upload function returns a promise
                    Api.putAlbum(data._id,{cover: resp.data.message}).success(function (){ 
                        stateCover = true;
        ...
        if( audio != null || audio != undefined){
          state = false;
           Cloud.upload({
            ...
            .then(function (resp) { //upload function returns a promise
                    Api.putAlbum(data._id,{audio: resp.data.message}).success(function (){ 
                        state = true;
        ...
        if( audio2 != null || audio2 != undefined){
          state2 = false;
           Cloud.upload({
            ...
            .then(function (resp) { //upload function returns a promise
                    Api.putAlbum(data._id,{audio_en: resp.data.message}).success(function (){ 
                        state2 = true;
                        console.log("img: "+stateImg+" cover :"+stateCover);
                        if(state2 && state && stateImg && stateCover){
          ...
          angular.forEach(file, function(value, key) {
            ...
            .then(function (resp) { //upload function returns a promise
                  ...
                  if($scope.Imgs.length == file.length){
                    Api.putAlbum(data._id,{images: $scope.Imgs}).success(function (){ 
                            stateImg = true;
                            if(state2 && state && stateImg && stateCover){
